...
GEMINI_MODEL_TEXT = "gemini-2.0-flash-exp"  # For text generation
...
if gemini_api_key:
    try:
        ...
        logger.warning(f"Failed to configure Gemini AI: {str(e)}")
else:
...
# Note: gemini_service and test_generator seem to be undefined - let's use the services we have
...
def create_comparison_agent():
    ...
    def analyze_brd(state):
        ...
        logger.info("Analyzing BRD content with Gemini")
        ...
        try:
            ...
            prompt = f"""
            You are a business analyst. Extract only the key functional requirements, features, and acceptance criteria from the BRD.
            
            Important Instructions:
            1. Focus ONLY on actual product functionality, features, and requirements
            2. Ignore document structure, headings, formatting, or any non-requirement content
            3. Organize your analysis by feature categories
            4. Present requirements as a numbered list for clarity
            5. Only include concrete, implementable requirements
            ...
            # Generate response with deterministic settings for consistency
            ...
            state["brd_analysis"] = response.text
            logger.info("BRD analysis completed successfully with Gemini")
            return state
        except Exception as e:
            error_msg = f"Gemini API error during BRD analysis: {str(e)}"
    ...
    def compare_brd_stories(state):
        ...
        logger.info("Comparing BRD analysis with user stories using Gemini")
        ...
        try:
            ...
            1. Focus ONLY on comparing actual features and functional requirements 
            2. Ignore document structure, headings, or any non-requirement content
            3. Only analyze substantive content representing actual product functionality
            
            Your analysis must include:

            1. A comparison between BRD functional requirements and user story features
               - Match each BRD feature with corresponding user stories that fulfill it
               - Be concise and direct

            2. A section with exactly this heading:

            ## MISSING REQUIREMENTS

            Under this heading, include a numbered list of specific functional requirements from the BRD that aren't addressed in any user story.
            - Use a tight, compact numbered list with NO blank lines between items
            - Each missing requirement should be an actual feature or capability
...
def create_direct_comparison_agent():
    ...
    def analyze_brd(state):
        brd_content = state.get("brd_content", "")
        logger.info("Analyzing BRD content with Gemini (direct comparison)")
    ...
    def compare_brd_stories(state):
        ...
        logger.info("Comparing BRD analysis with pre-processed user stories using Gemini")
...
async def compare_brd_stories(request: ComparisonRequest):
    try:
        ...
        try:
            ...
            # Detect domain and perform gap analysis
...
if __name__ == "__main__":
    logger.info("[START] Starting Test Recommendation Engine API v2.0")
    uvicorn.run(
        "main:app",
        host="0.0.0.0", 
        port=8000,
